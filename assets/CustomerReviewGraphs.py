...
class CustomerReviewAnalysis:
    ...
    class CustomerReviewGraphs:
        def getTimeSeries(self, data_most_reviewed_store):
            # Parse date strings into datetime objects
            dates = [datetime.strptime(date_str, '%Y-%m-%d') for date_str in data_most_reviewed_store['date']]

            # Format datetime objects into strings
            date_str = [date.strftime('%Y') for date in dates]
    ...
    def getTimeSeries(self, data_most_reviewed_store):
        ...
        # original
        # data_most_reviewed_store_timeseries_pos = \
        # data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 1].groupby('date_year')[
        #     'date_year', 'sentiments'].sum()

        #replaced
        data_most_reviewed_store_timeseries_pos = \
            data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 1].groupby('date_year')[
                ['date_year', 'sentiments']].sum(numeric_only=True)

        data_most_reviewed_store_timeseries_neg = \
        data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 3].groupby('date_year')[
            ['date_year', 'sentiments']].sum(numeric_only=True)
        data_most_reviewed_store_timeseries_neu = \
        data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 2].groupby('date_year')[
            ['date_year', 'sentiments']].sum(numeric_only=True)

        data_most_reviewed_store_timeseries_pos['rating'] = ['Positive' for i in
                                                             range(len(data_most_reviewed_store_timeseries_pos))]
        data_most_reviewed_store_timeseries_neg['rating'] = ['Negative' for i in
                                                             range(len(data_most_reviewed_store_timeseries_neg))]
        data_most_reviewed_store_timeseries_neu['rating'] = ['Neutral' for i in
                                                             range(len(data_most_reviewed_store_timeseries_neu))]
        data_most_reviewed_store_timeseries = pd.concat(
            [data_most_reviewed_store_timeseries_pos, data_most_reviewed_store_timeseries_neg])
        data_most_reviewed_store_timeseries = pd.concat(
            [data_most_reviewed_store_timeseries, data_most_reviewed_store_timeseries_neu])
    ...
    def getReviewAnalysis(self, review_business_data_merged, store_id):
        data_most_reviewed_store = review_business_data_merged[
            review_business_data_merged['business_id'] == store_id].sort_values(by=['date'])
        ...
        sentiment_fig = px.pie(data_most_reviewed_store, values='sentiments', names='sentiment_name',
                               color='sentiment_name',
                               color_discrete_map={'Neutral': 'yellow', 'Negative': 'cyan', 'Positive': 'green'})
