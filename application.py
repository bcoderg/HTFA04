...
data_most_reviewed_store_timeseries_pos = data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 1].groupby('date')[['date', 'sentiments']].sum(numeric_only=True)
#for numeric only in sum , removed this line and added the above line , UNDO in case of error
...
#inplace of this below line - kept above line , remove if errors occurs
#data_most_reviewed_store_timeseries_pos=data_most_reviewed_store[data_most_reviewed_store['sentiments']==1].groupby('date')['date','sentiments'].sum()
...
#updated
data_most_reviewed_store_timeseries_neg = data_most_reviewed_store[data_most_reviewed_store['sentiments'] == 3].groupby('date')[['date', 'sentiments']].sum(numeric_only=True)


data_most_reviewed_store_timeseries_neu=data_most_reviewed_store[data_most_reviewed_store['sentiments']==2].groupby('date')[['date', 'sentiments']].sum(numeric_only=True)
...
app.layout = html.Div([
    dbc.Card(dbc.CardBody([
        dbc.Row([
            dbc.Col(drawBusinessName(), width=3),
            dbc.Col(drawText(), width=3, id='sb01'),
            dbc.Col(drawTextOverallRating(), width=3, id='sb02'),
            dbc.Col(drawTotalReview(), width=3, id='sb03'),
        ], align='center'),
        html.Br(),
        dbc.Row([
            dbc.Col(drawPieFigure(), width=3, id='df02'),
            dbc.Col(drawTrendingWords(pos_keywords, neg_keywords), width=3, id='sb04'),
            dbc.Col(drawRecentReviews(recent_reviews), width=6, id='df04'),
        ], align='center'),
        html.Br(),
        dbc.Row([
            dbc.Col(drawTrendsBarFigure(), width=5, id='df03'),
            dbc.Col(drawTimeSeriesFigure(), width=4, id='df05'),
            dbc.Col(drawRatings(), width=3, id='df01'),
        ], align='center'),
    ]), color='dark')
])
...
@app.callback(
    ...
    Output(component_id='df01', component_property='children'),
    Output(component_id='df02', component_property='children'),
    Output(component_id='df04', component_property='children'),
    Output(component_id='df03', component_property='children'),
    Output(component_id='sb04', component_property='children')],
    [Input(component_id='business_name', component_property='value')]
)
